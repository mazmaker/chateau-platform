...
-- Update invite_user function to use correct search_path
DROP FUNCTION IF EXISTS invite_user(TEXT, TEXT, UUID, TEXT, UUID);
...
BEGIN
  ...
  -- Generate invite token using md5 random (works without pgcrypto)
  v_invite_token := encode(md5(random()::text || clock_timestamp()::text || p_email), 'base64');
