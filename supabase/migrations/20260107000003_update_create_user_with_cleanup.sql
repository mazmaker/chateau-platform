-- Update create_user_with_password to handle orphaned auth.users records
-- If email exists in auth.users but NOT in public.users, delete it first
...
-- Add comment
COMMENT ON FUNCTION create_user_with_password IS 'Create a new user with specified password. Automatically cleans up orphaned auth.users records.';
