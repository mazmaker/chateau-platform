-- Fix accept_invite_and_create_user - use the pending user's existing ID
...
BEGIN
  ...
  -- ID stays the same, just update status
  UPDATE users
  SET
    is_active = true,
    invite_token = NULL,
...
-- Add comment
COMMENT ON FUNCTION accept_invite_and_create_user IS 'Accept invite and create user with password. Uses existing ID from public.users to maintain consistency.';
